...
public class CodeGenVisitor extends Visitor
{
    ...
    private CodeGenerator codeGen; //the CodeGenerator we are visiting for

    //the class that we are currently compiling
     ...
     */
    public CodeGenVisitor(SymbolTable st, PrintWriter err, CodeGenerator codeGen)
    {
        super(err);
    ...
    {
        boolean oldALHS = assignLHS;
        assignLHS = false;
        //leave the target address on top of the stack
        node.visitTarget(this);
        assignLHS = oldALHS;

        FieldSymbol field = node.getSymbol();
        ClassSymbol target = node.getTargetSymbol();

        loadField(field, target, node);

        return null;
    }

    //common functionality used in visit FieldAccessNode and VarNode
    private void loadField(FieldSymbol field, ClassSymbol target, Node node)
    {
        int offset = target.getFieldOffset(field.getFieldNumber());
        if (offset < 0)
        {
            error("Field not found: " + field.getFieldNumber() + " '" + field.getName() + "'.", node);
            return;
        }

        writeStmt("ldc " + (offset+2));
        writeStmt("iadd");  //we now have the heap address on the stack

        if (!assignLHS)
        {
            writeStmt("aload 1");
            writeStmt("swap");
            writeStmt("iaload"); //and now we have the value on the top of the stack
        }
    }
    ...
    {
        //doesn't do much, all the outputing for classes is done in the getClassDecls() method
    ...
    public Object visit(SuperCallNode node)
    {
        ...
        int retAdd = pushPreMethodCall(node, 0);
        inExpr = oldExpr;
        ...
        //restore 'this' and local variables
        restoreVariables();
    ...
    {
        writeStmt("; for statement starts");
        String loopBegin = NumberGenerator.getInstance().getLabel();
        String loopTest = NumberGenerator.getInstance().getLabel();

        //output the initialisation statement
        writeStmt("; for_init");
        node.visitInitExpr(this);
        writeStmt("goto " + loopTest); //jump to the test part
        writeStmt(loopBegin + ":");    //   <----------+--------+
                                       //              |        |
        //visit the loop and increment parts           |        |
        writeStmt("; for_loop");
        ...
        writeStmt("; for_test");
        inExpr = true;
        node.visitTestExpr(this);      //                       |
        inExpr = false;
        writeStmt("ifne " + loopBegin); //if test passes, loop -+
    ...
    {
        writeStmt(";" + node.toString());
        //push 0 onto the stack and call System.exit() with this as the exit code
        writeStmt("iconst_0");
        writeStmt("invokestatic java/lang/System/exit(I)V");
        return null;
    }
    ...
    public Object visit(ConcatNode node)
    {
        ...
        //visit the left node and put it on the stack
        writeStmt(";lhs of ++");
        node.visitLeft(this);
        unknownToString(node.getLeftType());
        addToStringPool();
        writeStmt("istore 5");

        //visit the right node and push it onto the stack
        writeStmt(";rhs of ++");
        node.visitRight(this);
        unknownToString(node.getRightType());
        ...
        //create a new StringBuffer and push it onto the stack
        writeStmt("new java/lang/StringBuffer");
        writeStmt("dup"); //duplicate it
        writeStmt("invokenonvirtual java/lang/StringBuffer/<init>()V"); //call its constructor
        writeStmt("swap");
        writeStmt("invokevirtual java/lang/StringBuffer/append(Ljava/lang/String;)Ljava/lang/StringBuffer;");
